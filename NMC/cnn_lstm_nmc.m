%% CNN-LSTM NMC DATASET SOC ESTIMATION
clear; clc; close all;
rng(42)
...
%% 6. TRAINING OPTIONS (MATCHED WITH OTHER MODELS)
...
legend('True SoC','CNN-LSTM Predicted SoC')
title('CNN-LSTM SoC Prediction')
xlabel('Test Samples')
ylabel('SoC')
grid on
...
title('CNN-LSTM Regression Scatter Plot')
grid on
...
ylabel('Frequency')
title('CNN-LSTM Error Distribution')
grid on
...
xlabel('Test Samples')
ylabel('Error')
title('CNN-LSTM Error vs Samples')
grid on

%% SAVE MODEL

save('CNN_LSTM_Battery_Model.mat','net')